  ...
  const generateSchedule = () => { /* 省略せず記述 (v26: 自動生成にも外部負荷考慮を追加) */
    ...
    setTimeout(() => {
      ...
      config.dates.forEach(d => config.periods.forEach(p => config.classes.forEach(c => {
        const k = `${d}-${p}-${c}`; if (!schedule[k] || !schedule[k].subject) slots.push({ d, p, c, k });
      })));
      const currentCounts = {};
      ...
      const solve = (idx, tempSch, tempCnt) => {
        ...
        const sortedSubj = [...config.subjects].sort((a, b) => {
          const remA = (config.subjectCounts[a]||0) - (tempCnt[c][a]||0);
